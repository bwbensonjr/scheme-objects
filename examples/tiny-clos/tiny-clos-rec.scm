...
;
; This version of Tiny CLOS uses Chez Scheme's define-record-type for
; both instance and entity representations, replacing the original
; vector-with-hidden-tag approach.  Each record carries its class
; pointer directly and a separate vector of user slots, so indexed
; slot access stays O(1) and the +2 offset the vector version needed
; for its header goes away.  Records give us disjoint types with
; auto-generated predicates, so %instance? and %entity? no longer
; depend on an eq? sentinel tag.
;
; Entities must still be callable, which records are not, so
; %allocate-entity returns a closure and stores the entity record in
; an eq-hashtable keyed by that closure.
;

;
; instances
;
(define-record-type %instance-rec
  (fields (mutable class %instance-class %set-instance-class!)
          (immutable slots %instance-slots)))

(define %instance? %instance-rec?)

(define (%allocate-instance class nfields)
  (make-%instance-rec class (make-vector nfields #f)))

(define (%instance-ref instance index)
  (vector-ref (%instance-slots instance) index))

(define (%instance-set! instance index new-value)
...
; entities
;
; Entities are records like instances, but they must also be callable.
; %allocate-entity returns a closure that dispatches to the record's
; current proc; the closure-to-record mapping lives in an eq-hashtable
; so %entity? and the rest of the accessors can recover the record.
;
(define-record-type %entity-rec
  (fields (mutable proc   %entity-rec-proc    %entity-rec-proc-set!)
          (mutable class  %entity-rec-class   %entity-rec-class-set!)
          (immutable slots %entity-rec-slots)))

(define %entity-table (make-eq-hashtable))

(define (%entity-default-proc . args)
  (error '%entity "Called entity without first setting proc."))

(define (%allocate-entity class nfields)
  (let* ((rec (make-%entity-rec %entity-default-proc
                                class
                                (make-vector nfields #f)))
         (closure (lambda args (apply (%entity-rec-proc rec) args))))
    (hashtable-set! %entity-table closure rec)
    closure))

(define (%entity? x)
  (and (procedure? x)
       (hashtable-contains? %entity-table x)))

(define (%entity->rec closure)
  (hashtable-ref %entity-table closure #f))

(define (%entity-class closure)
  (%entity-rec-class (%entity->rec closure)))

(define (%set-entity-proc! closure proc)
  (%entity-rec-proc-set! (%entity->rec closure) proc))

(define (%entity-ref closure index)
  (vector-ref (%entity-rec-slots (%entity->rec closure)) index))

(define (%entity-set! closure index new-value)
  (vector-set! (%entity-rec-slots (%entity->rec closure)) index new-value))
...
;
; Customizable printing via record-writer.
;
; Tiny CLOS classes don't have a built-in name slot, so we keep display
; names in an eq-hashtable side table.  Use (set-class-name! <cls> 'name)
; to label a class, or (class-name <cls>) to fetch one.
;
; Printing itself is driven by the PRINT-OBJECT generic.  Specialize it
; with add-method to control how your instances are rendered:
;
;   (add-method print-object
;     (make-method (list <pos>)
;       (lambda (call-next-method obj port)
;         (display "#<pos " port)
;         (display (slot-ref obj 'x) port) (display ", " port)
;         (display (slot-ref obj 'y) port) (display ">" port))))
;
; The default method prints #<class-name> using the side-table name.
;
(define %class-names (make-eq-hashtable))

(define (class-name class)
  (hashtable-ref %class-names class #f))

(define (set-class-name! class name)
  (hashtable-set! %class-names class name)
  class)

(for-each (lambda (p) (set-class-name! (car p) (cdr p)))
  `((,<top>             . <top>)
    (,<object>          . <object>)
    (,<class>           . <class>)
...
(add-method print-object
  (make-method (list <top>)
    (lambda (call-next-method obj port)
      (display "#<" port)
      (display (or (class-name (class-of obj)) 'instance) port)
      (display ">" port))))
...
(add-method print-object
  (make-method (list <class>)
    (lambda (call-next-method obj port)
      (display "#<class " port)
      (display (or (class-name obj) 'anonymous) port)
      (display ">" port))))
...
(add-method print-object
  (make-method (list <generic>)
    (lambda (call-next-method obj port)
      ...
      (display (length (generic-methods obj)) port)
      (display " method(s)>" port))))

(add-method print-object
  (make-method (list <method>)
    (lambda (call-next-method obj port)
      (display "#<method (" port)
      (let loop ((ss (method-specializers obj)) (first? #t))
        (when (pair? ss)
          (unless first? (display " " port))
          (display (or (class-name (car ss)) 'anonymous) port)
          (loop (cdr ss) #f)))
      (display ")>" port))))

(record-writer (type-descriptor %instance-rec)
  (lambda (rec port wr)
    (print-object rec port)))

(record-writer (type-descriptor %entity-rec)
  (lambda (rec port wr)
    (display "#<entity-rec>" port)))
